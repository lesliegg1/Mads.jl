...
Mads.mads@info("Internal coupling using `Model` ...")
...
Mads.mads@info("Internal coupling using `Julia command` and `Templates` ...")
md = Mads.loadmadsfile(joinpath(workdir,  "internal-linearmodel+template.mads"))
tifor = Mads.forward(md)
Mads.mads@info("Internal coupling using `Julia command`, `Templates` and respecting template space...")
Mads.addkeyword!(md, "respect_space")
tifor = Mads.forward(md)
Mads.deletekeyword!(md, "respect_space")
Mads.mads@info("Internal coupling using `MADS model` ...")
md = Mads.loadmadsfile(joinpath(workdir, "internal-linearmodel-mads.mads"))
mfor = Mads.forward(md)
Mads.mads@info("External coupling using `Command`, `Templates` and `Instructions` ...")
...
Mads.mads@info("External coupling using `Command` and JLD ...")
...
Mads.mads@info("External coupling using `Command` and JSON ...")
...
if !haskey(ENV, "MADS_NO_PYTHON") && isdefined(:PyCall) && isdefined(Mads, :yaml)
	Mads.mads@info("External coupling using `Command` and YAML ...")
...
Mads.mads@info("External coupling using ASCII ...")
